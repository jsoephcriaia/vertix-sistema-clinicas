  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Visão geral da sua clínica</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="rounded-xl p-5 shadow-sm" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          ...
          <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{metricas.totalClientes}</p>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Clientes cadastrados</p>
        ...
        <div className="rounded-xl p-5 shadow-sm" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Pipeline</span>
          </div>
          <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>R$ {metricas.valorPipeline.toLocaleString('pt-BR')}</p>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Em negociação</p>
          <div className="flex gap-2 mt-2">
        ...
        <div className="rounded-xl p-5 shadow-sm" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Retornos</span>
          </div>
          <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{metricas.retornosSemana}</p>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Esta semana</p>
          <div className="flex gap-2 mt-2">
        ...
        <div className="rounded-xl p-5 shadow-sm" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Faturamento</span>
          </div>
          <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>R$ {metricas.faturamentoTotal.toLocaleString('pt-BR')}</p>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Total acumulado</p>
          <div className="flex gap-2 mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl p-5 shadow-sm" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          ...
          {leadsRecentes.length === 0 ? (
            <p className="text-sm text-center py-4" style={{ color: 'var(--text-secondary)' }}>Nenhum lead no pipeline</p>
          ) : (
            <div className="space-y-3">
              {leadsRecentes.map((lead) => (
                <div key={lead.id} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium" style={{ color: 'var(--text-primary)' }}>{lead.nome}</p>
                      <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{lead.interesse}</p>
                    </div>
                  </div>
        ...
        <div className="rounded-xl p-5 shadow-sm" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          ...
          {retornosProximos.length === 0 ? (
            <p className="text-sm text-center py-4" style={{ color: 'var(--text-secondary)' }}>Nenhum retorno agendado</p>
          ) : (
            <div className="space-y-3">
              {retornosProximos.map((cliente) => {
                ...
                return (
                  <div key={cliente.id} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold" style={{ backgroundColor: 'var(--primary)' }}>
                        {cliente.nome.charAt(0)}
                      </div>
                      <div>
                        <p className="font-medium" style={{ color: 'var(--text-primary)' }}>{cliente.nome}</p>
                        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{cliente.telefone}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{formatarData(cliente.proximo_retorno)}</p>
                      <p className={`text-xs ${diffDias === 0 ? 'text-yellow-600' : ''}`} style={{ color: diffDias === 0 ? undefined : 'var(--text-secondary)' }}>
        ...
        <div className="mt-6 bg-red-500/10 border border-red-500/30 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Acesse a tela de Retornos para entrar em contato com esses clientes.</p>
            </div>
          </div>
        </div>
      )}
